...
public class OnlineGame extends BaseGame {
    ...
    public OnlineGame(Context context, boolean useMusic, Handler handler) {
        super(context,useMusic,handler);
        this.backGround = ImageManager.BACKGROUND3_IMAGE;
        bossList = new LinkedList<>();
        if(this.usemusic){
            boss_bgm = new MyMediaPlayer(context,R.raw.bgm_boss);
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (isRunning) {
                    ...
                    bundle.putInt("selfScore",selfScore);
    ...
    public void postTask(int task) {
        if (backgroundHandler != null) {
            Message msg = Message.obtain();
            Bundle bundle = new Bundle();
            bundle.putInt("otherScore", task);
            msg.setData(bundle);
    ...
    protected void changeParameter() {
        ...
        this.selfScore = config.score;
        if ((boolean) resultmethod[0]) {
            Log.i(TAG,"游戏难度提升 所有的属性提升"+this.numada+"倍,周期变为原来的"+this.alpha+"倍");
            // 增加难度
                ...
                if (enemyAircraft.crash(bullet)) {
                    ...
                    if (enemyAircraft.notValid()) {
                        ...
                        if (enemyAircraft instanceof Boss ) {
                            ...
                            config.setBossPower(BossPower);
                            config.setBossSpeedx(BossSpeedx);
                            Log.i(TAG,"恭喜您成功击败BOSS！ 游戏难度提升： 最大敌机数量+1 " + this.enemyMaxNumber + "下一次Boss属性提升为" + this.beta + "倍" + "BOSS出现阈值变为" + this.gamma + "倍");
                        }
    ...
    public void draw() {
        ...
        paintOtherScoreAndLife(canvas,mPaint);
        paintBossHp(canvas,mPaint);
        mSurfaceHolder.unlockCanvasAndPost(canvas);
    }

    public void paintOtherScoreAndLife(Canvas canvas, Paint paint){
        int x = 10;
        int y = 300;
        ...
        canvas.drawText("OtherSCORE: " + otherScore, x, y, paint);
    ...
    protected void gameoverProcessAction() {
        if (heroAircraft.notValid()) {
            ...
            msg.what = 4;
            Bundle bundle = new Bundle();
            bundle.putInt("selfScore", this.selfScore);
            bundle.putBoolean("isRunning",isRunning);
            msg.setData(bundle);
